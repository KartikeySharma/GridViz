...
class DFS(rows: Int, cols: Int) {
    ...
    suspend fun solve(speed: Long, startX: Int, startY: Int, endX: Int, endY: Int, board: Array<IntArray>, ) {
    ...
    private suspend fun dfs(startX: Int, startY: Int, endX: Int, endY: Int,
        dx: ArrayList<Int>, dy: ArrayList<Int>, speed: Long, board: Array<IntArray>): Boolean {
        ...
        for (i in 0 until 4) {
            ...
            if (isValid(startX + dx[i], startY + dy[i], path, board)) {
                if (1 + dist[startX][startY] < dist[startX + dx[i]][startY + dy[i]]) {
                    ...
                    delay(speed)
                    when (i) {
                        0 -> { path[startY + dy[i]][startX + dx[i]] = 'U' }
                        1 -> { path[startY + dy[i]][startX + dx[i]] = 'L' }
                        2 -> { path[startY + dy[i]][startX + dx[i]] = 'D' }
                        else -> { path[startY + dy[i]][startX + dx[i]] = 'R' }
                    }
    ...
    private fun isValid(i: Int, j: Int, path: Array<CharArray>, board: Array<IntArray>): Boolean {
