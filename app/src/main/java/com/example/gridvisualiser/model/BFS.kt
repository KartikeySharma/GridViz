...
class BFS(rows: Int, cols: Int) {
    ...
    suspend fun solve( board: Array<IntArray>, startX: Int, startY: Int, endX: Int, endY: Int, speed: Long) {
            ...
            for (i in 0 until 4) {
                if (isValid(currX + dx[i], currY + dy[i], path, board)) {
                    if (1 + dist[currX][currY] < dist[currX + dx[i]][currY + dy[i]]) {
                        ...
                        when (i) {
                            0 -> { path[currY + dy[i]][currX + dx[i]] = 'U' }
                            1 -> { path[currY + dy[i]][currX + dx[i]] = 'L' }
                            2 -> { path[currY + dy[i]][currX + dx[i]] = 'D' }
                            else -> { path[currY + dy[i]][currX + dx[i]] = 'R' }
                        }
